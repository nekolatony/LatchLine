...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--context", choices=["0", "1"])
    parser.add_argument("--context-max-bytes")
    parser.add_argument("--context-depth")
...
def main() -> int:
    ...
    if args.context:
        overrides["REVIEWER_CONTEXT"] = args.context
    if args.context_max_bytes:
        overrides["REVIEWER_CONTEXT_MAX_BYTES"] = args.context_max_bytes
    if args.context_depth:
        overrides["REVIEWER_CONTEXT_DEPTH"] = args.context_depth
    if args.log_dir:
